...
describe("headless game env", () => {
  ...
  test("runs built-in and file-backed scenarios", () => {
    for (const name of ["smoke", "combat", "combat-skills", "status-effects", "character-name", "starting-loadout", "biome", "trap", "floor-modifier", "skill-check", "save-load", "save-management", "auth-local", "map-generation", "npc-event", "full-run"]) {
