...
opendungeon is a terminal roguelike RPG built around replayable dungeon layouts, local-first saves, and optional GM-created worlds. The engine owns the rules and state transitions. AI-assisted content belongs to logged-in GM worlds and enters the game as validated data patches, not as free-form runtime behavior.
...
- A deterministic RPG loop where the same dungeon code can replay the same floor layout and event placement.
