...
- [ ] Harden the accepted command stream into a fully host-applied command relay: host applies movement/combat/loot/village/tutorial commands to a server-owned game session, rejects invalid commands, broadcasts accepted results, and lets clients render from accepted host events instead of parallel local simulation.
...
- [ ] Persist generated world configs, archived player action logs, and AI-admin patches with owner/project boundaries in Supabase.
...
- [ ] Build `/gm` in `packages/www` as the logged-in GM console. The current page is logged-in, Supabase-backed, can draft via Vercel AI SDK + AI Gateway when configured, can fall back to rules validation, can read live host state, and can approve/deliver host patches. Remaining work is to replace the prompt form with the AI Elements chatbot-style transcript and richer tool-call UI.
