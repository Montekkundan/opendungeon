...
- [x] Keep a rendered disconnect dialog covered by tests so clients see that the host stopped and can return to title instead of continuing silently.
- [x] Show the disconnect dialog when a multiplayer command tries to send after the lobby socket is missing or closed, so a client cannot keep playing silently after the host exits.
- [x] Add a host-shutdown smoke test that proves connected WebSocket clients receive a close event when the `opendungeon-host` process stops.
...
- [x] Broadcast sanitized host inventory item names in command results so accepted loot, drop, potion, and utility commands can reconcile the local inventory screen instead of only syncing item counts.
- [x] Apply host-owned tutorial stage, readiness, and completion results back into the local co-op tutorial state so gates and coach UI do not drift after accepted host commands.
- [x] Broadcast and apply sanitized host village hub snapshots for station builds, food prep, shop sales, permissions, houses, farm state, and calendar fields so co-op village screens do not diverge after accepted host commands.
- [x] Broadcast and apply sanitized host progression snapshots for talents, level-up choices, equipment, Book entries, status effects, toasts, and recent log lines so accepted host commands can update more of the client UI from authoritative state.
- [x] Broadcast and apply sanitized host UI context snapshots for combat selection/rolls, talent-check modals, and NPC conversations so accepted co-op actions update the client's active modal/context from host state.
- [x] Broadcast and apply sanitized host dungeon snapshots for tiles, actors, anchors, secrets, player start, and floor modifier data so accepted co-op commands replace stale tutorial gates, walls, and actor placement with host-owned world state.
- [x] Store the full sanitized host command result as the lobby authoritative state so website and GM status surfaces can read the same host-owned inventory, progression, context, and world snapshots without replaying command history.
- [x] Preserve full host rollback and expose the latest host-owned command result per player so the host page and GM page can debug each connected crawler instead of only the latest global command.
- [x] Let clients apply their latest per-player host state snapshot even when the command entry has already been applied or trimmed, reducing local replay drift after reconnects and host snapshots.
- [x] Send pre-move coordinates with co-op movement commands so the host replays one intended step instead of hydrating to the optimistic client position and moving twice.
...
- [x] Verify trusted publishing and release workflow behavior after every changeset or package metadata change. Added `bun run release:verify` and included it in `bun run package:check` so the main workflow keeps Changesets versioning, npm Trusted Publishing permissions, package checks before publish, and the already-published npm guard. The verifier now also executes `release:npm` in dry-run mode so the previous `0.1.6` already-published failure remains covered.
- [x] Add a headless new-player journey scenario that covers inventory, Book, d20 checks, combat, tutorial handoff, first clear, village prep, next descent, and save/load.
...
- [x] Add cooking and crafting recipes that combine dungeon loot, farm crops, and trust unlocks into next-run food, bombs, tools, and charms.
